        ...
        <div className="absolute top-[80%] left-1/2 -translate-x-1/2">
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
                {CardContent.map((item) => (
                    <CardHome className="hidden lg:block xl:block" key={item.id} title={item.title} image={item.image} fill={item.fill} />
                ))}
                    ...
                    <Swiper spaceBetween={50} slidesPerView={1} modules={[Autoplay]} autoplay={{delay: 1000}}>
                        {CardContent.map((item) => (
                            <SwiperSlide key={item.id}>
                                ...
                                <p className="font-black text-5xl text-green-500">{item.fill}</p>
                            </SwiperSlide>
                        ))}
                    </Swiper>
                </div>
